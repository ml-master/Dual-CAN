...
def get_news(rootdir,start,end,label):
    ...
    for dir in dir_list[start:end]:
        ...
        Dict['text']=content['text'] # read news content.json
        entity_desc_list=[]
        try:
        ...
        Dict['tweet_list']=entity_desc_list
...
def get_data(i):
    ...
    tota,lista=get_news(rootdir,100*i,100*(i+1),'fake') # get 100 pieces of fake news
    rootdir='./fakenewsnet_dataset/gossipcop/real'
    totb,listb=get_news(rootdir,150*i,150*(i+1),'real') # get 150 pieces of real news
    lists=lista+listb # mix together
